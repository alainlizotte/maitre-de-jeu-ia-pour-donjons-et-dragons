...
def tools_prompt_compact(registry: dict[str, ToolSpec]) -> str:
    ...
    return (
        ...
        "(pas repos_long) — pour une POTION/FIOLE, passe "
        "`source=\"<nom exact de l'objet>\"` : la dose est déduite "
        "automatiquement de l'inventaire (refus si l'objet n'y est pas) ; "
        "pour un SORT, passe source=\"sort de soins\" (pas de consommable) ; "
        "le KIT de premiers secours consomme automatiquement 1 de ses 10 "
        "charges (épuisé → remplacé via inventaire_ajouter)\n"
        "- tir à distance (arc, arbalète, fronde, sarbacane, javelot) → "
        "la MUNITION (flèche, carreau, balle…) est déduite automatiquement "
        "par `lancer_attaque`\n"
        "- jet de sauvegarde → `lancer_sauvegarde`\n"
        "- gain d'XP → `fiche_perso_gagner_xp`\n"
        "- ramasser/ajouter un objet → `inventaire_ramasser` ou "
        "`inventaire_ajouter`\n"
        "- ACHETER au marché → `marche_consulter` / `marche_stock` puis "
        "`marche_acheter` (débite l'or de la fiche) — JAMAIS "
        "`inventaire_ajouter` pour un achat : le débit est obligatoire\n"
        "- VENDRE → `marche_vendre` (crédite l'or)\n"
        "- catalogue marchand / prix de base → `equipement_catalogue`\n"
        "- repas ou logement → `auberge_commander` (débite l'or, pose le "
        "marqueur de nuitée). PROPOSE d'abord les tarifs et ATTENDS la "
        "décision EXPLICITE du joueur : ne débite JAMAIS sans sa commande, "
        "ne le fais JAMAIS manger/dormir à sa place, même s'il entre dans "
        "l'auberge (partie e48e75dd : repas+nuit narrés sans commande)\n"
        "- familier / compagnon animal → `appeler_familier` (rituel ou "
        "lien), renvoi → `renvoyer_familier`\n"
